...
export const CardStack = () => {
  ...
  return (
    <section className="grid-in-current-card relative flex items-center justify-center">
      <AnimatePresence>
        <Card key={`front-${nextEvent?.id}`} event={currentEvent} />
      </AnimatePresence>
    </section>
  );
};

function Card({ event }: { event?: GameEvent }) {
  switch (event?.type) {
    case "music": {
      return <MusicalCard musicEvent={event} />;
    }
    case "historic": {
      return <HistoricalCard historicEvent={event} />;
    }
    default:
      return null;
  }
...
function HistoricalCard({ historicEvent }: { historicEvent?: HistoricEvent }) {
...
function MusicalCard({ musicEvent }: { musicEvent?: MusicEvent }) {
  const [isOpen, setIsOpen] = useState(false);
  const offscreenHeadingRef = useRef<HTMLHeadingElement>(null);
  const offscreenTextRef = useRef<HTMLParagraphElement>(null);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <aside className="opacity-0 pointer-events-none">
          <h2 ref={offscreenHeadingRef}>{musicEvent?.title}</h2>
          <p ref={offscreenTextRef}>{musicEvent?.artistName}</p>
        </aside>
      </motion.div>
    </motion.div>
  );
